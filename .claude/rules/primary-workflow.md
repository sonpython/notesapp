...
#### 0. Linear Tracking (MANDATORY)
- **Before starting:** Find or create a Linear issue for the task (`mcp__linear__*` tools, Team: Azdigi, Project: NotesApp)
- **Set status** → `In Progress` and assign to `me`
- **Pass Linear issue ID** to all subagents so they can comment/update
- **On completion:** Update status → `Done`, comment with summary
- **Full protocol:** See `.claude/rules/linear-integration.md`
...
- **LINEAR:** Tester agent MUST create Linear issues (label: `Bug`) for any failing tests with error details. On pass, comment on the feature issue.
...
- **LINEAR:** Reviewer agent MUST comment review summary on the feature issue. Create separate issues for critical findings (security, performance).
